                                    ...
                                    double ctrl_loop_rate)
{
  ...
  depth_sub_ = nh_.subscribe<geometry_msgs::PointStamped>("ms5837/depth", 1, &FlamingoController::depthCallback, this);
}
...
void FlamingoController::controlCore()
{
  ...
  //     if (underactuate_)
  //       target_wrench_acc_cog.head(3) = Eigen::Vector3d(target_acc_dash.x(), target_acc_dash.y(),
  //       target_acc_dash.z());
  //     else
  ...
  if (if_dive_)
  {
    ...
    double pid_depth = depth_p_gain_ * err_p + depth_i_gain_ * depth_err_i_ + depth_d_gain_ * err_d;
    double total_thrust = depth_hover_thrust_ + pid_depth;

    // Clamp thrust
    if (total_thrust > 10.0)
      total_thrust = 10.0;
    if (total_thrust < 0.0)
      total_thrust = 0.0;
