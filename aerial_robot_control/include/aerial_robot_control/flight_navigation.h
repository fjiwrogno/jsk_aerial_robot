...
/* control mode */
enum control_mode { POS_CONTROL_MODE, VEL_CONTROL_MODE, ACC_CONTROL_MODE };
/* control frame */
enum control_frame {
...
class BaseNavigator {
   public:
    ...
    inline void setTargetPos(double x, double y, double z) {
        setTargetPos(tf::Vector3(x, y, z));
    }
    inline void addTargetPos(tf::Vector3 diff_pos) { target_pos_ += diff_pos; }
    inline void addTargetPos(double x, double y, double z) {
        addTargetPos(tf::Vector3(x, y, z));
    }
    inline void setTargetVel(tf::Vector3 vel) { target_vel_ = vel; }
    inline void setTargetVel(double x, double y, double z) {
        setTargetVel(tf::Vector3(x, y, z));
    }
    inline void setTargetZeroVel() { setTargetVel(0, 0, 0); }
    inline void setTargetAcc(tf::Vector3 vel) { target_acc_ = vel; }
    inline void setTargetAcc(double x, double y, double z) {
        setTargetAcc(tf::Vector3(x, y, z));
    }
    ...
    inline void setTargetOmega(double x, double y, double z) {
        setTargetOmega(tf::Vector3(x, y, z));
    }
    ...
    inline void addTargetYaw(float value) {
        setTargetYaw(angles::normalize_angle(target_rpy_.z() + value));
    }
    ...
    inline void setTargetAngAcc(double x, double y, double z) {
        setTargetAngAcc(tf::Vector3(x, y, z));
    }
    ...
    static constexpr int PS4_AXIS_BUTTON_REAR_LEFT_2 =
        3;  // neutral=+1, full accel=-1
    static constexpr int PS4_AXIS_BUTTON_REAR_RIGHT_2 =
        4;  // neutral=+1, full accel=-1
    ...
    static constexpr int PS4_AXIS_BUTTON_CROSS_LEFT_RIGHT =
        9;  // left = +1, right= -1
    static constexpr int PS4_AXIS_BUTTON_CROSS_UP_DOWN =
        10;  // up = +1, down= -1
    ...
    static const sensor_msgs::Joy ps4joyToPs3joyConvert(
        const sensor_msgs::Joy& ps4_joy_msg);
   ...
   protected:
    ...
    double vel_nav_threshold_;  // the range (board) to siwtch between vel_nav
                                // and pos_nav
    ...
    void simpleMoveBaseGoalCallback(
    ...
    void startTakeoff() {
        ...
        double pos_x_error = getTargetPos().x() -
                             estimator_->getPos(Frame::COG, estimate_mode_).x();
        double pos_y_error = getTargetPos().y() -
                             estimator_->getPos(Frame::COG, estimate_mode_).y();
        double pos_xy_error_dist =
            std::sqrt(pos_x_error * pos_x_error + pos_y_error * pos_y_error);
        if (pos_xy_error_dist > takeoff_xy_pos_tolerance_) {
            ROS_ERROR_STREAM("initial xy error distance: "
                             << pos_xy_error_dist
        ...
        if (fabs(init_height_ -
                 estimator_->getPos(Frame::COG, estimate_mode_).z()) >
            takeoff_z_pos_tolerance_) {
            ROS_ERROR_STREAM(
                "difference between init height and current height: "
                << fabs(init_height_ -
                        estimator_->getPos(Frame::COG, estimate_mode_).z())
    ...
    void motorArming() {
        ...
        if (!estimator_->getStateStatus(State::Z_BASE, estimate_mode_)) {
            ROS_ERROR(
                "Flight Navigation: No correct sensor fusion for z(altitude), "
                "can not fly");
            return;
        }
        ...
        for (const auto& handler : estimator_->getGpsHandlers()) {
            if (handler->getStatus() == Status::ACTIVE) {
                ...
                nh.param("outdoor_hover_convergent_duration",
                         hover_convergent_duration_, 0.5);
                nh.param("outdoor_xy_convergent_thresh", xy_convergent_thresh_,
                         0.6);
                nh.param("outdoor_z_convergent_thresh", z_convergent_thresh_,
                         0.05);

                ROS_WARN_STREAM(
                    "update the navigation parameters for outdoor flight, "
                    "takeoff height: "
                    << takeoff_height_
                    << "; outdoor_hover_convergent_duration: "
                    << hover_convergent_duration_
                    << "; outdoor_xy_convergent_thresh: "
                    << xy_convergent_thresh_
                    << "; outdoor_z_convergent_thresh: "
                    << z_convergent_thresh_);
        ...
        ROS_INFO_STREAM("init height for takeoff: " << init_height_
                                                    << ", target height: "
                                                    << getTargetPos().z());
        ROS_INFO_STREAM("target xy pos: " << "[" << getTargetPos().x() << ", "
                                          << getTargetPos().y() << "]");
    ...
    tf::Vector3 frameConversion(tf::Vector3 origin_val, float yaw) {
        return frameConversion(origin_val,
                               tf::Matrix3x3(tf::createQuaternionFromYaw(yaw)));
    }

    void flightStatusAckCallback(const std_msgs::UInt8ConstPtr& ack_msg) {
        if (ack_msg->data ==
            spinal::FlightConfigCmd::ARM_OFF_CMD) {  //  arming off
            ROS_INFO("STOP RES From AERIAL ROBOT");
            setNaviState(ARM_OFF_STATE);
        }

        if (ack_msg->data ==
            spinal::FlightConfigCmd::ARM_ON_CMD) {  //  arming on
            ROS_INFO("START RES From AERIAL ROBOT");
            setNaviState(ARM_ON_STATE);
        }

        if (ack_msg->data ==
            spinal::FlightConfigCmd::
                FORCE_LANDING_CMD) {  //  get the first force landing message
                                      //  from spinal
    ...
    template <class T>
    void getParam(ros::NodeHandle nh, std::string param_name, T& param,
                  T default_value) {
        nh.param<T>(param_name, param, default_value);

        if (param_verbose_)
            ROS_INFO_STREAM("[" << nh.getNamespace() << "] " << param_name
                                << ": " << param);
    }
};
};  // namespace aerial_robot_navigation
