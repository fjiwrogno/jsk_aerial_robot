...
void AttitudeController::pwmsControl(void)
{
  ...
  void AttitudeController::fourAxisCommandCallback(const spinal::FourAxisCommand& cmd_msg)
  {
    ...
    if (fabs(cmd_msg.angles[0]) > MAX_TILT_ANGLE || fabs(cmd_msg.angles[1]) > MAX_TILT_ANGLE)
    {
      setForceLandingFlag(true);
#ifdef SIMULATION
      ROS_ERROR("failsafe: target angles are too large, roll: %f (%f), pitch: %f ()%f", target_angle_[X],
                MAX_TILT_ANGLE, target_angle_[Y], MAX_TILT_ANGLE);
#else
  ...
  void AttitudeController::pwmTestCallback(const spinal::PwmTest& pwm_msg)
  {
    ...
    if (pwm_msg.motor_index_length)
    {
      ...
      for (int i = 0; i < pwm_msg.motor_index_length; i++)
      {
        ...
        else if ((i > 1) && pwm_msg.pwms[i] >= 0 && pwm_msg.pwms[i] <= MAX_PWM)
        {
  ...
  void AttitudeController::pwmConversion()
  {
    ...
    for (int i = 0; i < motor_number_ / rotor_coef_; i++)
    {
      ...
      {
        case 2:
          thrust = ap::pythagorous3(base_thrust_term_[rotor_coef_ * i] + roll_pitch_term_[rotor_coef_ * i],
                                    base_thrust_term_[rotor_coef_ * i + 1] + roll_pitch_term_[rotor_coef_ * i + 1],
                                    base_thrust_term_[rotor_coef_ * i + 2] + roll_pitch_term_[rotor_coef_ * i + 2]);
          break;
        case 1:
          thrust = ap::pythagorous2(base_thrust_term_[rotor_coef_ * i] + roll_pitch_term_[rotor_coef_ * i],
                                    base_thrust_term_[rotor_coef_ * i + 1] + roll_pitch_term_[rotor_coef_ * i + 1]);
          break;
        case 0:
        ...
        if (max_yaw_term_index_ != -1 && fabs(base_thrust_term_[0]) > 0)
        {
          ...
          for (int i = 0; i < motor_number_ / (rotor_coef_); i++)
          {
            ...
            switch (gimbal_dof_)
            {
              case 2:
                thrust =
                    ap::pythagorous3(base_thrust_term_[rotor_coef_ * i] + roll_pitch_term_[rotor_coef_ * i],
                                     base_thrust_term_[rotor_coef_ * i + 1] + roll_pitch_term_[rotor_coef_ * i + 1],
                                     base_thrust_term_[rotor_coef_ * i + 2] + roll_pitch_term_[rotor_coef_ * i + 2]);
                break;
              case 1:
                thrust =
                    ap::pythagorous2(base_thrust_term_[rotor_coef_ * i] + roll_pitch_term_[rotor_coef_ * i],
                                     base_thrust_term_[rotor_coef_ * i + 1] + roll_pitch_term_[rotor_coef_ * i + 1]);
                break;
              case 0:
                thrust = base_thrust_term_[i] + roll_pitch_term_[i];
        ...
        switch (gimbal_dof_)
        {
          case 2: {
            ...
            float gimbal_candidate_pitch =
                atan2f(f_i.x, -f_i.y * sin(gimbal_candidate_roll) + f_i.z * cos(gimbal_candidate_roll));
