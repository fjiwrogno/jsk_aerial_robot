...
void AttitudeController::init(ros::NodeHandle* nh, StateEstimate* estimator) {
    ...
    control_feedback_state_pub_ =
        nh_->advertise<spinal::RollPitchYawTerm>("rpy/feedback_state", 1);
    anti_gyro_pub_ = nh_->advertise<std_msgs::Float32MultiArray>(
        "gyro_moment_compensation", 1);
    four_axis_cmd_sub_ =
        nh_->subscribe("four_axes/command", 1,
                       &AttitudeController::fourAxisCommandCallback, this);
    pwm_info_sub_ = nh_->subscribe("motor_info", 1,
                                   &AttitudeController::pwmInfoCallback, this);
    rpy_gain_sub_ = nh_->subscribe("rpy/gain", 1,
                                   &AttitudeController::rpyGainCallback, this);
    p_matrix_pseudo_inverse_inertia_sub_ =
        nh_->subscribe("p_matrix_pseudo_inverse_inertia", 1,
                       &AttitudeController::pMatrixInertiaCallback, this);
    pwm_test_sub_ = nh_->subscribe("pwm_test", 1,
                                   &AttitudeController::pwmTestCallback, this);
    att_control_srv_ = nh_->advertiseService(
        "set_attitude_control", &AttitudeController::setAttitudeControlCallback,
        this);
    torque_allocation_matrix_inv_sub_ = nh_->subscribe(
        "torque_allocation_matrix_inv", 1,
        &AttitudeController::torqueAllocationMatrixInvCallback, this);
    sim_vol_sub_ = nh_->subscribe("set_sim_voltage", 1,
                                  &AttitudeController::setSimVolCallback, this);
    offset_rot_sub_ = nh_->subscribe(
        "desire_coordinate", 1, &AttitudeController::offsetRotCallback, this);
    baseInit();
    gimbal_control_pub_ =
        nh_->advertise<sensor_msgs::JointState>("gimbals_ctrl", 1);
}
...
AttitudeController::AttitudeController()
    : pwms_pub_("motor_pwms", &pwms_msg_),
      ...
      control_feedback_state_pub_("rpy/feedback_state",
                                  &control_feedback_state_msg_),
      four_axis_cmd_sub_("four_axes/command",
                         &AttitudeController::fourAxisCommandCallback, this),
      ...
      torque_allocation_matrix_inv_sub_(
          ...
          &AttitudeController::torqueAllocationMatrixInvCallback, this),
      offset_rot_sub_("desire_coordinate",
                      &AttitudeController::offsetRotCallback, this),
      att_control_srv_("set_attitude_control",
                       &AttitudeController::setAttitudeControlCallback, this),
      esc_telem_pub_("esc_telem", &esc_telem_msg_) {}

void AttitudeController::init(TIM_HandleTypeDef* htim1,
                              TIM_HandleTypeDef* htim2,
                              StateEstimate* estimator, DShot* dshot,
                              DirectServo* servo, BatteryStatus* bat,
    ...
    if (!dshot_) {
        ...
        // Configure TIM1 for 330Hz PWM, optimal for high-speed digital servos
        // and ESCs. Timer Clock = 200MHz. Tick Period = 1us. PWM Period
        // = 3.03ms (~330Hz).
        pwm_htim1_->Init.Prescaler = 199;
        pwm_htim1_->Init.CounterMode =
            TIM_COUNTERMODE_UP;  // Use standard UP mode
        pwm_htim1_->Init.Period = 3029;
        ...
        // --- Configure Channels 1 & 2 for bidirectional neutral throttle
        // (1.5ms) ---
        sConfigOC.Pulse = 1500;
        while (HAL_TIM_PWM_ConfigChannel(pwm_htim1_, &sConfigOC,
                                         TIM_CHANNEL_1) != HAL_OK);
        while (HAL_TIM_PWM_ConfigChannel(pwm_htim1_, &sConfigOC,
                                         TIM_CHANNEL_2) != HAL_OK);

        // --- Configure Channels 3 & 4 for standard minimum throttle (1.0ms)
        // ---
...
void AttitudeController::pwmsControl(void) {
    ...
    if (HAL_GetTick() - control_term_pub_last_time_ >
        CONTROL_TERM_PUB_INTERVAL) {
        control_term_pub_last_time_ = HAL_GetTick();
        control_term_pub_.publish(control_term_msg_);
    }

    if (HAL_GetTick() - control_feedback_state_pub_last_time_ >
        CONTROL_FEEDBACK_STATE_PUB_INTERVAL) {
    ...
    if (dshot_) {
        ...
        for (int i = 0; i < 4; i++) {
            // target_pwm_: 0.5 ~ 1.0
            uint16_t motor_v =
                (uint16_t)((target_pwm_[i] - 0.5) / 0.5 * DSHOT_RANGE +
                           DSHOT_MIN_THROTTLE);
        ...
        if (dshot_->is_telemetry_) {
            if (dshot_->esc_reader_.is_update_all_msg_) {
                ...
                float voltage_ave =
                    (float)(dshot_->esc_reader_.esc_msg_1_.voltage +
                            dshot_->esc_reader_.esc_msg_2_.voltage +
                            dshot_->esc_reader_.esc_msg_3_.voltage +
                            dshot_->esc_reader_.esc_msg_4_.voltage) /
                    400.0;
        ...
        // This block translates the logical PWM value (target_pwm_)
        // into a physical pulse width in microseconds (us) for each channel.
        // The timer is configured with a 1us tick, so the CCR value is
        // numerically equal to the desired pulse width in microseconds.

        // --- Channels 1 & 2: Bi-directional Thrusters ---
        // Translates a logical PWM [0.0, 1.0] to a physical pulse [1000us,
        // 2000us].
        for (int i = 0; i < 2; ++i) {
            float pulse_bi = 1000.0f + target_pwm_[i] * 1000.0f;
            if (pulse_bi < 1000.0f) pulse_bi = 1000.0f;
            if (pulse_bi > 2000.0f) pulse_bi = 2000.0f;

            if (i == 0) {
                pwm_htim1_->Instance->CCR1 = (uint32_t)pulse_bi;
            } else {
                pwm_htim1_->Instance->CCR2 = (uint32_t)pulse_bi;
            }
        }

        // --- Channels 3 & 4: Uni-directional Motors ---
        // Translates a logical PWM [0.5, 1.0] to a physical pulse [1000us,
        // 2000us].
        for (int i = 2; i < 4; ++i) {
            float remapped_pwm = (target_pwm_[i] - 0.5f) * 2.0f;
            if (remapped_pwm < 0.0f) remapped_pwm = 0.0f;
            if (remapped_pwm > 1.0f) remapped_pwm = 1.0f;

            float pulse_uni = 1000.0f + remapped_pwm * 1000.0f;

            if (i == 2) {
                pwm_htim1_->Instance->CCR3 = (uint32_t)pulse_uni;
            } else {
                pwm_htim1_->Instance->CCR4 = (uint32_t)pulse_uni;
            }
        }
    }
#if !GX_PWM_SERVO
    pwm_htim2_->Instance->CCR1 =
        (uint32_t)(target_pwm_[4] * pwm_htim2_->Init.Period);
    pwm_htim2_->Instance->CCR2 =
        (uint32_t)(target_pwm_[5] * pwm_htim2_->Init.Period);
    pwm_htim2_->Instance->CCR3 =
        (uint32_t)(target_pwm_[6] * pwm_htim2_->Init.Period);
    pwm_htim2_->Instance->CCR4 =
        (uint32_t)(target_pwm_[7] * pwm_htim2_->Init.Period);
#endif
...
void AttitudeController::update(void) {
    ...
    if (start_control_flag_ && att_control_flag_) {
        ...
        if (failsafe_ && !force_landing_flag_ &&
            (int32_t)(HAL_GetTick() - flight_command_last_stamp_) >
                FLIGHT_COMMAND_TIMEOUT) {
            /* timeout => start force landing */
#ifdef SIMULATION
            ROS_ERROR(
                "failsafe: cannot connect with ROS. time now: %d, time last "
                "stamp: %d",
                HAL_GetTick(), flight_command_last_stamp_);
#else
        ...
        if (!force_landing_flag_ && (fabs(angles[X]) > MAX_TILT_ANGLE ||
                                     fabs(angles[Y]) > MAX_TILT_ANGLE)) {
#ifdef SIMULATION
            ROS_ERROR(
                "failsafe: the roll pitch angles are too large, roll: %f (%f), "
                "pitch: %f (%f)",
        ...
        // linear control method
        {
            ...
            for (int axis = 0; axis < 3; axis++) {
                ...
                if (axis == X) {
                    control_feedback_state_msg_.roll_p =
                        error_angle[axis] * 1000;
                    control_feedback_state_msg_.roll_i =
                        error_angle_i_[axis] * 1000;
                    control_feedback_state_msg_.roll_d = vel[axis] * 1000;
                }
                if (axis == Y) {
                    control_feedback_state_msg_.pitch_p =
                        error_angle[axis] * 1000;
                    control_feedback_state_msg_.pitch_i =
                        error_angle_i_[axis] * 1000;
            ...
            for (int i = 0; i < motor_number_; i++) {
                for (int axis = 0; axis < 3; axis++) {
                    ...
                    if (axis == Z) {
                        ...
                        control_term_msg_.motors[i].yaw_d =
                            d_term * 1000;  // d_term;
                    }
                }
        ...
        if (force_landing_flag_) {
            ...
            if (average_thrust > force_landing_thrust_) {
                for (int i = 0; i < motor_number_; i++)
                    base_thrust_term_[i] -=
                        (base_thrust_term_[i] / average_thrust *
                         FORCE_LANDING_INTEGRAL);
            }
        }
    }
...
void AttitudeController::fourAxisCommandCallback(
    const spinal::FourAxisCommand& cmd_msg) {
    if (!start_control_flag_) return;  // do not receive command

    /* failsafe: if the pitch and roll angle is too big, start force landing */
    if (fabs(cmd_msg.angles[0]) > MAX_TILT_ANGLE ||
        fabs(cmd_msg.angles[1]) > MAX_TILT_ANGLE) {
        setForceLandingFlag(true);
#ifdef SIMULATION
        ROS_ERROR(
            "failsafe: target angles are too large, roll: %f (%f), pitch: %f "
            "()%f",
    ...
    if (cmd_msg.base_thrust.size() != motor_number_) {
        ROS_ERROR(
            "fource axis commnd: motor number is not identical between fc(%d) "
            "and pc(%ld)",
            motor_number_, cmd_msg.base_thrust.size());
        return;
    }
#else
    if (cmd_msg.base_thrust_length != motor_number_) {
        nh_->logerror(
            "fource axis commnd: motor number is not identical between fc and "
            "pc");
        return;
    }
#endif
    ...
    for (int i = 0; i < motor_number_; i++) {
        ...
        if (max_yaw_term_index_ != -1)
            extra_yaw_pi_term_[i] = cmd_msg.angles[Z] * thrust_d_gain_[i][Z] /
                                    thrust_d_gain_[max_yaw_term_index_][Z];
    }
...
void AttitudeController::rpyGainCallback(
    const spinal::RollPitchYawTerms& gain_msg) {
    if (motor_number_ == 0) return;  // not be activated

    /* check the number of motor which should be equal to the ros thrust */
#ifdef SIMULATION
    if (gain_msg.motors.size() != motor_number_ &&
        gain_msg.motors.size() != 1) {
        ROS_ERROR(
            "rpy gain: motor number is not identical between fc:%d and pc:%d",
            motor_number_, (int)gain_msg.motors.size());
        return;
    }
#else
    if (gain_msg.motors_length != motor_number_ &&
        gain_msg.motors_length != 1) {
        nh_->logerror(
            "rpy gain: motor number is not identical between fc and pc");
        return;
    }
#endif
...
void AttitudeController::torqueAllocationMatrixInvCallback(
    const spinal::TorqueAllocationMatrixInv& msg) {
    if (motor_number_ == 0 || !start_control_flag_) return;

#ifdef SIMULATION
    if (msg.rows.size() != motor_number_) {
        ROS_ERROR(
            "torqueAllocationMatrixInvCallback: motor number is not identical "
            "between fc(%d) and pc(%ld)",
...
void AttitudeController::thrustGainMapping() {
    for (int i = 0; i < motor_number_; i++) {
        thrust_p_gain_[i][X] =
            torque_allocation_matrix_inv_[i][X] * torque_p_gain_[X];
        thrust_i_gain_[i][X] =
            torque_allocation_matrix_inv_[i][X] * torque_i_gain_[X];
        thrust_d_gain_[i][X] =
            torque_allocation_matrix_inv_[i][X] * torque_d_gain_[X];
        thrust_p_gain_[i][Y] =
            torque_allocation_matrix_inv_[i][Y] * torque_p_gain_[Y];
        thrust_i_gain_[i][Y] =
            torque_allocation_matrix_inv_[i][Y] * torque_i_gain_[Y];
        thrust_d_gain_[i][Y] =
            torque_allocation_matrix_inv_[i][Y] * torque_d_gain_[Y];
        thrust_d_gain_[i][Z] =
            torque_allocation_matrix_inv_[i][Z] * torque_d_gain_[Z];
    }
}
...
void AttitudeController::pwmTestCallback(const spinal::PwmTest& pwm_msg) {
    ...
    if (pwm_msg.motor_index_length) {
        ...
        if (pwm_msg.motor_index_length != pwm_msg.pwms_length) {
            nh_->logerror(
                "The number of index does not match the number of pwms.");
            return;
        }
        for (int i = 0; i < pwm_msg.motor_index_length; i++) {
            ...
            } else if ((i <= 1) && pwm_msg.pwms[i] >= 0 &&
                       pwm_msg.pwms[i] <= MAX_PWM) {
                pwm_test_value_[motor_index] = pwm_msg.pwms[i];
            } else {
...
void AttitudeController::setMotorNumber(uint16_t motor_number) {
    ...
    if (motor_number_ > 0) {
        ...
        control_term_msg_.motors =
            new spinal::RollPitchYawTerm[control_term_msg_size];
#endif
...
void AttitudeController::pMatrixInertiaCallback(
    const spinal::PMatrixPseudoInverseWithInertia& msg) {
    ...
    if (msg.pseudo_inverse.size() != motor_number_) {
        if (motor_number_ > 0)
            ROS_ERROR(
                "p matrix pseudo inverse and inertia commnd: motor number is "
                "not identical between fc and pc");
        return;
    }
#else
    if (msg.pseudo_inverse_length != motor_number_) {
        nh_->logerror(
            "p matrix pseudo inverse and inertia commnd: motor number is not "
            "identical between fc and pc");
        return;
    }
#endif
    ...
    /* inertia */
    inertia_ = ap::Matrix3f(msg.inertia[0] * 0.001f, msg.inertia[3] * 0.001f,
                            msg.inertia[5] * 0.001f, msg.inertia[3] * 0.001f,
                            msg.inertia[1] * 0.001f, msg.inertia[4] * 0.001f,
                            msg.inertia[5] * 0.001f, msg.inertia[4] * 0.001f,
                            msg.inertia[2] * 0.001f);
...
void AttitudeController::pwmConversion() {
    if (!start_control_flag_ && !pwm_test_flag_) {
        return;  // Exit immediately, preserving the safe initial PWM values.
    }
            ...
            case spinal::MotorInfo::SQRT_MODE: {
                ...
                float sqrt_tmp =
                    ...
                    4 * 10 * motor_info_[motor_ref_index_].polynominal[2] *
                        (motor_info_[motor_ref_index_].polynominal[0] -
                         scaled_thrust);  // special decimal order shift (x10)
                target_pwm = (-motor_info_[motor_ref_index_].polynominal[1] +
                              sqrt_tmp * ap::inv_sqrt(sqrt_tmp)) /
                             (2 * motor_info_[motor_ref_index_].polynominal[2]);
                break;
            }
            case spinal::MotorInfo::POLYNOMINAL_MODE: {
                /* pwm = F_inv[(V_ref / V)^1.5 f] */
                float tenth_scaled_thrust =
                    scaled_thrust * 0.1f;  // special decimal order shift (x0.1)
                /* 4 dimensional */
                int max_dimenstional = 4;
                target_pwm =
                    motor_info_[motor_ref_index_].polynominal[max_dimenstional];
                for (int j = max_dimenstional - 1; j >= 0; j--)
                    target_pwm = target_pwm * tenth_scaled_thrust +
                                 motor_info_[motor_ref_index_].polynominal[j];
                break;
            }
            default: {
                break;
    ...
    if (HAL_GetTick() - voltage_update_last_time_ > 500)  //[500ms = 0.5s]
    {
        ...
        switch (pwm_conversion_mode_) {
            ...
            case spinal::MotorInfo::POLYNOMINAL_MODE: {
                ...
                v_factor_ = motor_info_[motor_ref_index_].voltage / voltage *
                            ap::inv_sqrt(voltage /
                                         motor_info_[motor_ref_index_].voltage);
                break;
            }
            default: {
                break;
    ...
    /* get the decreasing rate for the thrust to avoid the divergence because of
     * the pwm saturation */
    ...
    for (int i = 0; i < motor_number_ / rotor_coef_; i++) {
        ...
        switch (gimbal_dof_) {
            case 2:
                thrust =
                    ap::pythagorous3(base_thrust_term_[rotor_coef_ * i] +
                                         roll_pitch_term_[rotor_coef_ * i],
                                     base_thrust_term_[rotor_coef_ * i + 1] +
                                         roll_pitch_term_[rotor_coef_ * i + 1],
                                     base_thrust_term_[rotor_coef_ * i + 2] +
                                         roll_pitch_term_[rotor_coef_ * i + 2]);
                break;
            case 1:
                thrust =
                    ap::pythagorous2(base_thrust_term_[rotor_coef_ * i] +
                                         roll_pitch_term_[rotor_coef_ * i],
                                     base_thrust_term_[rotor_coef_ * i + 1] +
                                         roll_pitch_term_[rotor_coef_ * i + 1]);
                break;
    ...
    if (start_control_flag_) {
        ...
        if (residual_term < 0 && base_thrust_term_[max_thrust_index] > 0) {
            base_thrust_decreasing_rate =
                residual_term /
                (base_thrust_term_[max_thrust_index] / rotor_devider_);
            yaw_decreasing_rate =
                -1;  // also, we have to ignore the yaw control
        } else {
            if (max_yaw_term_index_ != -1 && fabs(base_thrust_term_[0]) > 0) {
                ...
                for (int i = 0; i < motor_number_ / (rotor_coef_); i++) {
                    ...
                    switch (gimbal_dof_) {
                        case 2:
                            thrust = ap::pythagorous3(
                                base_thrust_term_[rotor_coef_ * i] +
                                    roll_pitch_term_[rotor_coef_ * i],
                                base_thrust_term_[rotor_coef_ * i + 1] +
                                    roll_pitch_term_[rotor_coef_ * i + 1],
                                base_thrust_term_[rotor_coef_ * i + 2] +
                ...
                float residual_term_max =
                    thrust_limit - max_thrust / rotor_devider_;
                float residual_term_min =
                    min_thrust / rotor_devider_ - min_thrust_;
                int thrust_index = 0;
                ...
                if (residual_term < 0) {
                    yaw_decreasing_rate =
                        residual_term /
                        (fabs(yaw_term_[thrust_index]) / rotor_devider_);
                }
    ...
    for (int i = 0; i < motor_number_; i++)
        target_thrust_[i] =
            roll_pitch_term_[i] +
            (1 + base_thrust_decreasing_rate) * base_thrust_term_[i] +
        ...
        if (start_control_flag_) {
            switch (gimbal_dof_) {
                case 2: {
                    ...
                    float gimbal_candidate_pitch =
                        atan2f(f_i.x, -f_i.y * sin(gimbal_candidate_roll) +
                                          f_i.z * cos(gimbal_candidate_roll));
                    target_thrust_[i] = ap::pythagorous3(f_i.x, f_i.y, f_i.z);

                    /* simple lpf */
                    if (std::isfinite(gimbal_candidate_roll) &&
                        std::isfinite(gimbal_candidate_pitch)) {
                        target_gimbal_angles_[2 * i] =
                            (target_gimbal_angles_[2 * i] +
                             gimbal_candidate_roll) /
                            2;
                        target_gimbal_angles_[2 * i + 1] =
                            (target_gimbal_angles_[2 * i + 1] +
                             gimbal_candidate_pitch) /
                            2;
                    }
                    break;
                }
                case 1: {
                    ...
                    if (std::isfinite(gimbal_candidate))
                        target_gimbal_angles_[i] =
                            (target_gimbal_angles_[i] + gimbal_candidate) / 2;
    ...
    switch (gimbal_dof_) {
        case 2: {
            ...
            for (int i = 0; i < motor_number_ / (rotor_coef_); i++) {
                gimbal_control_msg.position.push_back(
                    target_gimbal_angles_[2 * i]);
                gimbal_control_msg.position.push_back(
                    target_gimbal_angles_[2 * i + 1]);
            }
