...
#ifndef SPINAL
      if (servo_[i].external_encoder_flag_)
        encoder_handler_.setOffset(servo_[i].joint_offset_);
#endif
    }
  }
...
void DynamixelSerial::setRoundOffset(uint8_t servo_index, int32_t ref_value)
{
  ServoData& s = servo_[servo_index];
  if (s.torque_enable_)
    return;  // cannot process if torque is enable
...
void DynamixelSerial::setHomingOffset(uint8_t servo_index)
{
  ...
  {
    servo_[servo_index].joint_offset_ =
        servo_[servo_index].calib_value_ + servo_[servo_index].joint_offset_ - servo_[servo_index].present_position_;
#ifndef SPINAL
...
void DynamixelSerial::update()
{
  ...
  if (read_status_packet_flag_)
  {
    for (unsigned int i = 0; i < servo_num_; i++)
    {
      if (!servo_[i].send_data_flag_ && !servo_[i].first_get_pos_flag_)
        continue;
  ...
  {
    if (set_pos_tick_ == 0)
      set_pos_tick_ = current_time + SET_POS_OFFSET;  // init
    else
      set_pos_tick_ = current_time;
  ...
  {
    if (get_pos_tick_ == 0)
      get_pos_tick_ = current_time + GET_POS_OFFSET;  // init
    else
      get_pos_tick_ = current_time;
  ...
  {
    if (get_load_tick_ == 0)
      get_load_tick_ = current_time + GET_LOAD_OFFSET;  // init
    else
      get_load_tick_ = current_time;
  ...
  {
    if (get_temp_tick_ == 0)
      get_temp_tick_ = current_time + GET_TEMP_OFFSET;  // init
    else
      get_temp_tick_ = current_time;
  ...
  {
    if (get_move_tick_ == 0)
      get_move_tick_ = current_time + GET_MOVE_OFFSET;  // init
    else
      get_move_tick_ = current_time;
  ...
  {
    if (get_error_tick_ == 0)
      get_error_tick_ = current_time + GET_HARDWARE_ERROR_STATUS_OFFSET;  // init
    else
      get_error_tick_ = current_time;
    ...
    if (ttl_rs485_mixed_ != 0)
    {
      switch (instruction.first)
      {
        case INST_GET_PRESENT_POS: /* read servo position(angle) */
          if (!servo_[servo_index].send_data_flag_ && !servo_[servo_index].first_get_pos_flag_)
            break;
          cmdReadPresentPosition(servo_index);
...
void DynamixelSerial::transmitInstructionPacket(uint8_t id, uint16_t len, uint8_t instruction, uint8_t* parameters)
{
  ...
  HAL_UART_Transmit(huart_, transmit_data, transmit_data_index,
                    10);  // timeout: 10 ms. Although we found 2 ms is enough OK for our case by oscilloscope. Large
                          // value is better for UART async task in RTOS.
}
    ...
    case INST_GET_PRESENT_POS: {
      ...
      if (s != servo_.end())
      {
        ...
        if (s->external_encoder_flag_)
        {
          ...
          {
            s->present_position_ = (int32_t)(encoder_handler_.getValue());  // use external encoder value instead of
                                                                            // servo internal encoder value
          ...
          switch (status_packet_instruction)
          {
            ...
            case INST_GET_PRESENT_POS: {
              ...
              if (s != servo_.end())
              {
                ...
                if (s->external_encoder_flag_)
                {
                  ...
                  {
                    s->present_position_ =
                        (int32_t)(encoder_handler_.getValue());  // use external encoder value instead of
                                                                 // servo internal encoder value
                  ...
                  {
                    s->hardware_error_status_ |= 1
                                                 << ENCODER_CONNECT_ERROR;  // |= 0b10000000:  encoder is not connected
                  }
#else
                  return 0;
#endif
            ...
            case INST_GET_HOMING_OFFSET:
              if (s != servo_.end())
              {
                s->homing_offset_ = ((parameters[3] << 24) & 0xFF000000) | ((parameters[2] << 16) & 0xFF0000) |
                                    ((parameters[1] << 8) & 0xFF00) | (parameters[0] & 0xFF);
              }
              return 0;
            case INST_GET_HARDWARE_ERROR_STATUS:
              if (s != servo_.end())
              {
                s->hardware_error_status_ &=
                    ((1 << RESOLUTION_RATIO_ERROR) + (1 << ENCODER_CONNECT_ERROR));  //  &= 0b11000000;
            ...
            case INST_GET_PROFILE_VELOCITY:
              if (s != servo_.end())
              {
                s->profile_velocity_ = ((parameters[3] << 24) & 0xFF000000) | ((parameters[2] << 16) & 0xFF0000) |
                                       ((parameters[1] << 8) & 0xFF00) | (parameters[0] & 0xFF);
              }
              return 0;
            default:
        ...
        void DynamixelSerial::cmdSyncRead(uint16_t address, uint16_t byte_size, bool send_all)
        {
          ...
          for (unsigned int i = 0; i < servo_num_; i++)
          {
            if (!send_all && !servo_[i].send_data_flag_ && !servo_[i].first_get_pos_flag_)
              continue;
