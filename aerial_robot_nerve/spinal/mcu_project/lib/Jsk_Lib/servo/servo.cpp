...
void DirectServo::servoTorqueControlCallback(
    const spinal::ServoTorqueCmd& control_msg) {
...
void DirectServo::servoConfigCallback(
    const spinal::SetDirectServoConfig::Request& req,
    spinal::SetDirectServoConfig::Response& res) {
    // TODO: using boardConfigCallback in spine.cpp
    uint8_t command = req.command;

    /* special case : data[0] is flag value */
    if (command ==
        spinal::SetDirectServoConfig::Request::SET_DYNAMIXEL_TTL_RS485_MIXED) {
    ...
    switch (command) {
        case spinal::SetDirectServoConfig::Request::SET_SERVO_HOMING_OFFSET: {
            if (!s.torque_enable_) {
                ...
                nh_->logerror(
                    "Cannot set homing offset during torque on state.");
                res.success = false;
            }
            break;
        }
        ...
        case spinal::SetDirectServoConfig::Request::
            SET_SERVO_EXTERNAL_ENCODER_FLAG: {
            if (!s.torque_enable_) {
                s.external_encoder_flag_ = req.data[1];
                s.first_get_pos_flag_ = true;
                if (!s.external_encoder_flag_) {  // if use the servo internal
                                                  // encoder, we directly output
                                                  // the encoder value without
                                                  // scaling by
                                                  // resolution_ratio.
                ...
                nh_->logerror(
                    "Cannot set ex encoder falg during torque on state.");
                res.success = false;
            }
            break;
        }
        case spinal::SetDirectServoConfig::Request::
            ...
            if (!s.torque_enable_) {
                ...
                s.hardware_error_status_ &=
                    ((1 << RESOLUTION_RATIO_ERROR) - 1);  // 0b00111111: reset

                if (s.servo_resolution_ == 65535 ||
                    s.joint_resolution_ == 65535) {
                    s.hardware_error_status_ |=
                        (1 << RESOLUTION_RATIO_ERROR);  // 0b01000000;
                    s.resolution_ratio_ = 1;
                } else {
                    s.resolution_ratio_ =
                        (float)s.servo_resolution_ / (float)s.joint_resolution_;
                    s.first_get_pos_flag_ = true;
                    FlashMemory::erase();
                    FlashMemory::write();
                }
                ...
                nh_->logerror(
                    "Cannot set resolution rate during torque on state.");
                res.success = false;
            }
...
void DirectServo::boardInfoCallback(const spinal::GetBoardInfo::Request& req,
                                    spinal::GetBoardInfo::Response& res) {
    // TODO: Bad implementation. This features should not be located in servo
    // interface.
...
void DirectServo::jointProfilesCallback(
    const spinal::JointProfiles& joint_prof_msg) {
    for (unsigned int i = 0; i < joint_prof_msg.joints_length; i++) {
        ...
        joint_profiles_[i].zero_point_offset =
            joint_prof_msg.joints[i].zero_point_offset;
    }
}