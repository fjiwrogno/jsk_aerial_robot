...
{
  if (servo_[servo_index].force_servo_off_)
    return;

  if (servo_[servo_index].torque_enable_ == torque_enable)
    return;
  if (torque_enable)
  {
...
void DynamixelSerial::update()
{
  ...
  {
    if (set_command_tick_ == 0)
      set_command_tick_ = current_time + SET_COMMAND_OFFSET;  // init
    else
      set_command_tick_ = current_time;
...
int8_t DynamixelSerial::readStatusPacket(uint8_t status_packet_instruction)
{
  ...
  switch (status_packet_instruction)
  {
    ...
    case INST_GET_HARDWARE_ERROR_STATUS:
      if (s != servo_.end())
      {
        s->hardware_error_status_ &= ((1 << PULLEY_SKIP_ERROR) + (1 << RESOLUTION_RATIO_ERROR) +
                                      (1 << ENCODER_CONNECT_ERROR));  //  &= 0b11000010;
